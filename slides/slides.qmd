...
author: "A seminar for wiiw · The Vienna Institute for International Economic Studies"
date: last-modified
format:
  revealjs:
...
- **Korinek (2023, *JEL*)** — founding taxonomy. LLMs assist across the whole pipeline; usefulness rises as **verification cost falls**. *Verification cost is the organizing variable.*
- **Korinek (2025, NBER)** — the shift this seminar is about: **chatbots → agents**. An agent is *model + tools + memory + iteration*.
- **Dell (2025, *JEL*)** — deep learning as **data creation**: OCR, embeddings, linkage unlock trapped sources.
- **Bahoo et al. (2025)** — the adoption map across subfields.
- **Sant'Anna (2026)** — the practitioner's missing manual. *Deployed in Part 2.*
...
- **Athey, Keleher & Spiess (2025)** — predicting *who is worst off* ≠ predicting *who responds*. Whom to nudge is **causal**.
- **Gavrilova et al. (2025)** — causal forests *inside* a DiD design: heterogeneity from the algorithm, **identification from the design**.
- **Chen, Cheng, Liu & Tang (2026)** — theory-guided transfer learning: structure regularizes a flexible learner; biggest gains in **small samples**.
...
- **Acemoglu (2025)** — sober task-based benchmark: cumulative TFP gains **< 1pp** over a decade. A defensible lower bound.
- **Jones (2026)** — the growth-theory counterweight. *Weak links* cap gains at the hard human tasks — yet automating *nearly everything* still delivers large acceleration. The debate is about **timing and bottlenecks**, not direction.
- **Brynjolfsson, Korinek & Agrawal (2026)** — the research agenda if AI is transformative. A menu of wiiw-sized questions: CESEE convergence, labour reallocation, nearshoring.
- **Mullainathan (2025)** — algorithms now sit *inside* the economy as decision-makers. **The profession itself is part of the treatment group.**
...
::: {.src}
A pleasant aside: the workflow guide and the modern DiD estimator in `did` **share an author** — the tooling and the econometrics come from the same intellectual household.
:::

## The money moment {.smaller}

One request, planning first: *"Extend the sample through 2025, refresh Figure 2, rerun the event study, update the results paragraph."*

1. Agent enters **plan mode** → you read it aloud → **approve.** *This is the moment of delegation.*
2. Execution runs inside the **permission fence** — narrate what it allows and blocks.

::: {.catch}
**Verification catches a planted bug** — a results number typed by hand that no longer matches the refreshed table. The critic flags it, the fixer repairs it, the render passes.

**This is the slide where the skeptics convert** — the pipeline caught what a tired human on a deadline would miss.
:::

Then: `git diff` · quality score · **commit.** Hold up the compiled PDF.
...
::: {.catch}
**Clone it this afternoon:** `github.com/lusiki/AI_workshop_wiiw` — paste the starter prompt, produce a compiled section on your own machine. A 90-minute hands-on clinic follows within two weeks.
:::
...
::: {.cols}
::: {.col}
- Korinek (2023, *JEL*) — founding taxonomy — *A*
- Korinek (2025, NBER) — chatbots → agents — *A / 2*
- Dell (2025, *JEL*) — DL as data creation — *A*
- Bahoo et al. (2025) — adoption map — *A*
- Sant'Anna (2026) — the practitioner manual — *2 / 3*
- Ludwig, Mullainathan & Rambachan (2025) — two valid uses — *B / 5*
- Athey, Keleher & Spiess (2025) — nudge whom — *B*
:::
::: {.col}
- Gavrilova et al. (2025) — forests inside DiD — *B*
- Chen, Cheng, Liu & Tang (2026) — theory-guided TL — *B*
- Brynjolfsson, Li & Raymond (2025, *QJE*) — field evidence — *hook / 5*
- Chatterji et al. (2025) — usage at scale — *hook*
- Acemoglu (2025) — sober benchmark — *C*
- Jones (2026) — weak links vs acceleration — *C / 5*
- Brynjolfsson, Korinek & Agrawal (2026) — the agenda — *C*
- Mullainathan (2025) — objects & instruments — *bridge*
:::
:::
